<template>
  <div>
    <div
      ...
      @keypress="isRefundInfoOpen = !isRefundInfoOpen"
      class="success-gas-refund-table-info-container"
    >
      <div class="gas-refund-title">
        <div class="gas-refund-text-img-container">
          <span>
            Gas Refund +{{$utils.formatMoney(gasRefundPercents, 2)}}%
            (${{$utils.formatMoney(gasRefundInUsd, 2)}})
            <img
      ...
      <div v-if="isRefundInfoOpen">
        ...
        <div>
          <div class="input-token-container">
            ...
            <div class="success-data-list">
              <span class="success-data-item">
                -{{$utils.formatMoney(netEthCost, 6) }}
                ETH (${{$utils.formatMoney(netEthCost * zksyncFeeHistory.ethPrice, 2)}})
              </span>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    netEthCost() {
      ...
      return this.zksyncFeeHistory.estimateFeeInEther
      - (this.zksyncFeeHistory.startWeiBalance - this.zksyncFeeHistory.finalWeiBalance);
    },
    refundEth() {
      if (!this.zksyncFeeHistory) {
        return 0;
