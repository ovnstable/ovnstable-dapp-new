...
export const useTokensQueryNew = (tokenService: ITokenService, stateData: any) => {
  ...
  // const getTokenListForNetwork = () => (tokensQuery.data.value
  //   ? tokensQuery.data.value.chainTokenMap[networkId.value as string].tokenMap : []);

  const getTokenList = () => Object.keys(
    tokensQuery.data.value
      ? mergeTokenLists(tokensQuery.data.value.chainTokenMap)
        .chainTokenMap[networkId.value].tokenMap : {},
  );
