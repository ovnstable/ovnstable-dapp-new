  ...
  computed: {
    ...
    entryFee() {
      if (this.etsStrategyData[this.etsData.name] && this.etsStrategyData[this.etsData.name].fees) {
        const result = this.etsStrategyData[this.etsData.name].fees.find((x) => x.id === 'buy');
    ...
    resultMintFee() {
      const entryFeeValue = this.sum * (this.entryFee ? (this.entryFee / 100.0) : 0);
    ...
    buttonLabel() {
      ...
      if (!this.account) {
      ...
      } if (!this.isOvercapAvailable && this.etsData.prototype) {
        return 'ETS is in prototype'
      } if (this.etsData.disabled) {
        return 'Unavailable'
      } if (this.transactionPending) {
        return 'Transaction is pending';
      } if (parseFloat(this.sum) < this.resultMintFee) {
        ...
        if (this.isOvercapAvailable) {
          if (this.account && this.sum > 0 && this.numberRule) {
            const noOvercapSum = parseFloat(this.etsData.maxSupply) - parseFloat(this.totalSupply[this.etsData.name]);
            let useOvercapSum;
        ...
        if ((this.etsData.maxSupply && this.totalSupply[this.etsData.name] >= this.etsData.maxSupply) || (this.etsData.maxSupply && (parseFloat(this.totalSupply[this.etsData.name]) + parseFloat(this.sum)) >= parseFloat(this.etsData.maxSupply))) {
          return 'Over ETS capacity';
        } if (this.sum > parseFloat(this.actionAssetBalance[`${this.etsData.actionAsset  }_${  this.etsData.actionTokenDecimals}`])) {
          return 'Mint ETS';
        } else {
          return 'Mint ETS';
        }
  ...
  methods: {
    ...
    async checkGalxeNft() {
      ...
      try {
        ...
        const abiFile = await loadJSON(`/contracts/${this.networkName}/Nft.json`);
        const nftContract = new this.web3.eth.Contract(abiFile.abi, abiFile.address);

        const isNftExist = await nftContract.methods.balanceOf(account).call() * 1;
    ...
    isNumber(evt) {
      ...
      if ((charCode > 31 && (charCode < 48 || charCode > 57)) && charCode !== 46) {
        evt.preventDefault();
      } else if (charCode === 46 && (!this.sum || this.sum.includes('.'))) {
        evt.preventDefault();
      } else {
        return true;
      }
